...
import { MAX_TOWERS, getItemUpgradeCost, getItemUpgradeChance } from '../src/game/superUnits';
...
function ingredients(type: TowerType): UnitInstance[] { const role = type === 'archer' ? 'archer' : type === 'warrior' ? 'single' : type === 'mage' ? 'area' : 'support'; return [baseUnique[type], ...['legendary', 'hero', 'epic'].flatMap(r => Array(3).fill(`${r}-${role}`))].map((definitionId, i) => ({ instanceId: `u${i}`, definitionId, x: 120 + i * 8, y: 148, cooldownMs: 0, attackUpgradePercent: 2, speedUpgradePercent: 1, upgradeCount: 1 })); }
...
describe('super-unique towers and dragon equipment', () => {
    test('rare units cannot replace epic ingredients and are preserved on crafting', () => {
        const sim = game();
        const recipe = ingredients('warrior');
        const rares = recipe.slice(-3).map((unit, i) => ({ ...unit, instanceId: `rare-${i}`, definitionId: 'rare-single' }));
        sim.state.gold = 10000;
        sim.state.board = [...recipe.slice(0, -3), ...rares];
        const before = JSON.stringify(sim.state);
        expect(sim.craftSuper('warrior')).toBe(false);
        expect(JSON.stringify(sim.state)).toBe(before);
        sim.state.board.push(...recipe.slice(-3));
        expect(sim.craftSuper('warrior')).toBe(true);
        expect(sim.state.board.filter(unit => unit.definitionId === 'rare-single')).toEqual(rares);
        expect(sim.state.board.some(unit => unit.definitionId === 'epic-single')).toBe(false);
        expect(sim.state.board).toHaveLength(4);
    });
    test('blessings only appear after fifth waves, excluding the final victory', () => { for (const wave of [1, 2, 3, 4, 5, 6, 10, 120]) {
        const sim = game();
        sim.state.wave = wave - 1;
        sim.state.baseHealth = sim.state.maxBaseHealth = 1e6;
        sim.startNextWave();
        sim.update(sim.waves[wave - 1]!.durationMs);
        expect(sim.pendingReward).toBe(wave % 5 === 0);
        if (wave === 120)
            expect(sim.state.status).toBe('running');
    } });
    test('speed dice increases firing speed, consumes gold once and survives merge', () => { const sim = game(); sim.state.board = ingredients('warrior').slice(1, 4); sim.state.gold = 1000; const before = sim.getTowerCombatStats(0)!.attackSpeed; const roll = sim.rollTowerUpgrade(0, 'haste')!; expect(roll.stat).toBe('haste'); sim.resolveUpgradeRoll(); expect(sim.getTowerCombatStats(0)!.attackSpeed).toBeLessThan(before); const total = sim.state.board.reduce((n, u) => n + (u.speedUpgradePercent ?? 0), 0); const prompt = sim.requestMerge(0)!; sim.chooseMergeCandidate(prompt.candidates[0]!.id); expect(sim.state.board[0]!.speedUpgradePercent).toBe(total); });
    ...
    test.each(['archer', 'warrior', 'mage', 'priest'] as const)('crafts exactly one %s super using the exact recipe and inherits upgrades', type => { const sim = game(); sim.state.board = ingredients(type); sim.state.gold = 10000; expect(sim.craftSuper(type)).toBe(true); expect(sim.state.gold).toBe(0); expect(sim.state.board).toHaveLength(1); expect(sim.state.board[0]).toMatchObject({ definitionId: `super-${type}`, attackUpgradePercent: 20, speedUpgradePercent: 10 }); expect(getUnitDefinition(sim.state.board[0]!.definitionId).superUnique).toBe(true); expect(sim.meta.uniqueUnitLevels[`super-${type}`]).toBe(1); sim.state.gold = 10000; sim.state.board.push(...ingredients(type)); expect(sim.craftSuper(type)).toBe(false); });
    test('missing or wrong-class ingredients and insufficient gold never consume anything', () => { for (const gold of [9999, 10000]) {
        const sim = game();
    ...
    test('common berserk lasts five seconds, waits three, and doubles attack and speed', () => { const sim = game(); sim.state.board = [{ instanceId: 's', definitionId: 'super-warrior', cooldownMs: 1e6, x: 150, y: 148 }]; const base = sim.getTowerCombatStats(0)!; sim.startNextWave(); const active = sim.getTowerCombatStats(0)!; expect(active.attack).toBeCloseTo(base.attack * 2); expect(active.attackSpeed).toBeCloseTo(base.attackSpeed / 2); sim.update(5000); expect(sim.isSuperBerserk(sim.state.board[0]!)).toBe(false); sim.update(3000); expect(sim.isSuperBerserk(sim.state.board[0]!)).toBe(true); });
    test.each(['archer', 'warrior'] as const)('%s hits five targets and deals five times damage to bosses', type => { const sim = game(); sim.state.board = [{ instanceId: 's', definitionId: `super-${type}`, cooldownMs: 0, x: 195, y: 148 }]; for (let i = 0; i < 6; i++)
    ...
    test('weapon +7 adds automatic magic damage and shoes reroll within bounds per wave', () => { const sim = game(); sim.state.board = [{ instanceId: 's', definitionId: 'super-warrior', x: 195, y: 148, cooldownMs: 0, items: [{ kind: 'weapon', level: 7, bonus: 0 }, { kind: 'boots', level: 0, bonus: 0 }] }]; sim.startNextWave(); expect(sim.state.board[0]!.items![1]!.waveSpeedPercent).toBeGreaterThanOrEqual(10); expect(sim.state.board[0]!.items![1]!.waveSpeedPercent).toBeLessThanOrEqual(80); sim.enemies.splice(0); sim.enemies.push(enemy('magic-target', 0.1)); sim.update(1); expect(sim.drainEvents()).toContainEqual(expect.objectContaining({ type: 'superSkill', skill: 'dragon-magic' })); });
});
