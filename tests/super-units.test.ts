...
function ingredients(type: TowerType): UnitInstance[] { const role = type === 'archer' ? 'archer' : type === 'warrior' ? 'single' : type === 'mage' ? 'area' : 'support'; return [baseUnique[type], ...['legendary', 'mythic', 'transcendent', 'immortal'].flatMap(r => Array(3).fill(`${r}-${role}`))].map((definitionId, i) => ({ instanceId: `u${i}`, definitionId, x: 120 + i * 8, y: 148, cooldownMs: 0, attackUpgradePercent: 2, speedUpgradePercent: 1, upgradeCount: 1 })); }
