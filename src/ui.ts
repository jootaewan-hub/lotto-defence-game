...
export function createUi(sim: GameSimulation, persistProgress = true): UiHandle {
  ...
  <footer><span>${icon('moon')} 달빛 수호대</span><p>Q 소환 <i>·</i> W 고급 소환 <i>·</i> E 결계 <i>·</i> Space 일시정지</p><span>성장 기록 자동 저장</span></footer>
    ...
    button('help-button', () => open('help', heading('플레이 가이드', '운명은 뽑고, 전술은 고르세요', '소환 · 배치 · 합성, 세 가지로 시작하는 달빛의 전투') + `<div class="guide-list"><article><b>01</b><div><h3>수호자를 모으세요</h3><p>시작 수호자 3명이 배치되어 있습니다. 골드로 동료를 소환하고 원정 시작을 누르세요.</p></div></article><article><b>02</b><div><h3>길목을 지키세요</h3><p>수호자를 드래그해 이동합니다. 클릭하면 사거리가 보입니다. 기사·마법사·사제를 함께 편성하면 공격력 +15%.</p></div></article><article><b>03</b><div><h3>합성하고, 결계를 펼치세요</h3><p>같은 수호자 3명을 합성하면 상위 등급을 직접 고릅니다. 달빛 결계는 현재 적을 3초간 얼립니다. 기본 재사용 24초. 선택한 타워는 골드로 공격력 룰렛 또는 공격속도 룰렛 강화(+0.2~5%)를 강화합니다. 합성 시 수치가 계승됩니다. 슈퍼유니크 각성에는 같은 유형 유니크 1명·전설/신화/초월/불멸 각 3명과 10,000G가 필요합니다.</p></div></article><article><b>04</b><div><h3>네 난이도의 120웨이브를 돌파하세요</h3><p>보통 → 나이트메어 → 헬 → Insane 순서로 각각 120웨이브를 진행합니다. 난이도 전환 시 수호대와 강화가 유지됩니다. 보통 대비 적 체력·방어력은 나이트메어 +50%(1.5배), 헬 +100%(2배), Insane +200%(3배)이며 Insane은 등장 수가 2배입니다. 최대 수호대는 200명입니다. 적은 순환 경로를 돌고, 제한시간 종료 시 생존한 일반 적 6명당 체력 1, 보스당 5를 잃습니다. 5웨이브마다 중간보스, 10웨이브마다 보스, 30웨이브마다 진보스가 등장합니다. 10웨이브마다 25종 중 무작위 축복 3개를 제시하며, 하나를 골라 주사위로 강화량을 정합니다. 실패해도 성장 조각은 남습니다.</p></div></article></div><button data-close class="dialog-primary">이제 지킬 준비가 됐어요</button>`));
    ...
    function render() {
        ...
        const bossBars = bosses.slice(0, 2).map(e => ({label: `${e.trueBossId ? '진보스' : e.wave % 10 === 0 ? '보스' : '중간보스'} · ${e.variantLabel}`, hp: Math.max(0, e.hp), maxHp: e.maxHp}));
        ...
        q('phase-chip').textContent = ended ? (s.status === 'won' ? '방어 성공' : '성채 함락') : sim.isWaveActive ? (sim.activeWaveDefinition?.isTrueBoss ? '진보스 습격' : sim.activeWaveDefinition?.isBoss ? '보스 습격' : '전투 진행 중') : s.wave ? '다음 습격 준비' : '수호대 배치 중';
        ...
        q('roster-hint').textContent=s.board.length>=MAX_TOWERS?'정원이 가득 찼습니다. 타워를 선택해 골드 강화하거나 합성하세요.':'불멸 3개 → 유니크 · 유니크 동시 보유 최대 2명';
