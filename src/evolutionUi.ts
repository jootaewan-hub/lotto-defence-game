import { ULTIMATE_ID, ULTIMATE_COST, ULTIMATE_SKILLS } from './game/ultimate';
import type { UnitDefinition } from './game/types';
import { RARITIES, getRarity } from './game/rarities';
/** The generic evolution ladder. Unique has no generic units, so it is not a rung. */
const LADDER = RARITIES.filter(r => r.id !== 'unique');
import { getTowerType, getUnitPortrait, getUnitDefinition } from './game/units';
import { TOWER_TYPES, TOWER_LABELS, SUPER_COST, SUPER_INGREDIENT_COUNT } from './game/superUnits';
...
export function evolutionPathMarkup(unit: UnitDefinition): string {
    ...
    return `<div class="evolution-path" aria-label="진화 단계">${LADDER.map((r, i) => `<span class="${!unit.superUnique && i + 1 === visual.stage ? 'current' : ''}" style="--step-color:${r.color}">${i + 1} ${r.label}</span>`).join('<i>›</i>')}</div><p class="evolution-note">같은 수호자 3명 합성 · 슈퍼유니크는 별도 재료로 최종 각성</p>`;
}

export function evolutionGalleryMarkup(): string {
    const roles = { archer: 'archer', warrior: 'single', mage: 'area', priest: 'support' };
    return `<span class="dialog-eyebrow">수호자 진화 도감</span><h2 id="dialog-title">작은 수호자에서, 전장의 전설로</h2><p class="dialog-description">숫자는 등급 진화 단계입니다. 같은 수호자 3명을 합성하면 다음 등급으로 진화합니다. 유니크는 별도의 레벨과 스킬 진화도 성장합니다.</p>${TOWER_TYPES.map(type => `<section class="evolution-gallery"><h3>${TOWER_LABELS[type]}</h3><div class="evolution-gallery-grid">${LADDER.map((r, i) => `<article>${unitArtMarkup(getUnitDefinition(`${r.id}-${roles[type]}`))}<b style="color:${r.color}">${i + 1}단계 · ${r.label}</b></article>`).join('')}</div><div class="awakening-preview">${unitArtMarkup(getUnitDefinition(`super-${type}`))}<div><small>별도 조합 · 최종 각성</small><h4>${getUnitDefinition(`super-${type}`).name}</h4><p>같은 유형 유니크 1 + 전설 ${SUPER_INGREDIENT_COUNT} + 신화 ${SUPER_INGREDIENT_COUNT} + 초월 ${SUPER_INGREDIENT_COUNT} + 불멸 ${SUPER_INGREDIENT_COUNT} + ${SUPER_COST.toLocaleString()}G</p></div></div></section>`).join('')}<section class="awakening-preview ultimate-preview">${unitArtMarkup(getUnitDefinition(ULTIMATE_ID))}<div><small>사신 합일 · 궁극 각성</small><h4>무극신</h4><p>슈퍼유니크 4종 + ${ULTIMATE_COST.toLocaleString()}G</p>${ULTIMATE_SKILLS.map(s=>`<p><b>${s.name}</b> · ${s.description}</p>`).join('')}</div></section><button data-close class="dialog-primary">전장으로 돌아가기</button>`;
}
